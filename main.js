  ...
  elPaymentPeriodRadio.addEventListener('change', function(){
    ...
    if (period === 'annual') {
      elsPricingPlanValue.forEach(function(elPrice) {
        elPrice.textContent = elPrice.closest('.site-main-section').dataset.paymentAnnual;
      });
    } else {
      elsPricingPlanValue.forEach(function (elPrice) {
        elPrice.textContent = elPrice.closest('.site-main-section').dataset.paymentMonthly;
      });
    }
  });
});